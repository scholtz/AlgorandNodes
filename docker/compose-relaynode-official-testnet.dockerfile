...
RUN apt update && apt dist-upgrade -y && apt install -y mc wget telnet dnsutils git curl iotop atop vim && apt-get clean autoclean && apt-get autoremove --yes && rm -rf /var/lib/{apt,dpkg,cache,log}/
