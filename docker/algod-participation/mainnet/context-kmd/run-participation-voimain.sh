...
blockchainRoundMath=$(curl -s https://mainnet-api.voi.nodely.dev/v2/status | jq -r '.["last-round"]' )" - 1000"
...
if [ "$myNodeRound" -lt "$blockchainRound" ]; then
	...
	catchup=$(curl -s https://mainnet-api.voi.nodely.dev/v2/status | jq -r '.["last-catchpoint"]')
	sleep 10s
